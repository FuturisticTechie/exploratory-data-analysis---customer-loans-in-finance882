...
class RDSDatabaseConnector:
    ...
    def load_table(self): 
        self.engine = self.init_db_engine()                                 #initialises db connection/ accesses tables in db/ reads data from 2nd table
        with self.engine.begin() as conn:
            self.loan_payments_df = pd.read_sql_table('loan_payments', conn)    #Loads it as dataframe
        return self.loan_payments_df   


    def save_dataframe_to_csv(self, df, output_file):
